...
let cmd_dvipng tmpprefix finalpath = "dvisvgm -e -n " ^ tmpprefix ^ ".dvi -o " ^ finalpath ^ " >/dev/null 2>/dev/null"
...
let render tmppath finalpath outtex md5 backcolor =
    ...
    let f = (Util.open_out_unless_exists (tmpprefix ^ ".tex")) in
      begin
        ...
        ) else if (Sys.command (cmd_dvipng tmpprefix (finalpath^"/"^md5^".svg")) != 0)
          then (
